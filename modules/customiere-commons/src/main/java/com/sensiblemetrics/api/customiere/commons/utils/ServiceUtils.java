package com.sensiblemetrics.api.customiere.commons.utils;
...
public class ServiceUtils {
    ...
    /**
     * Returns generic parameter {@link Class} {@link P} by input {@link Class} instance
     *
     * @param clazz The class to introspect
     * @return the first generic declaration, or Object.class if cannot be determined
     */
    @Nullable
    public static <T, P> Class<P> getClassGenericType(final Class<T> clazz) {
        return getClassGenericType(clazz, 0);
    }

    /**
     * Returns generic parameter {@link Class} {@link P} by input {@link Class} instance and {@code int} index
     *
     * @param clazz - initial input {@link Class} instance to fetch from
     * @param index - initial input {@code int} index to fetch by
     * @return the index generic declaration, or Object.class if cannot be determined
     */
    @Nullable
    public static <T, P> Class<P> getClassGenericType(final Class<T> clazz, final int index) {
        final Type genericType = clazz.getGenericSuperclass();
        if (!(genericType instanceof ParameterizedType)) {
            log.warn(clazz.getSimpleName() + "'s superclass not ParameterizedType");
            return null;
        }
        final Type[] params = ((ParameterizedType) genericType).getActualTypeArguments();
