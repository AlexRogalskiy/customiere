...
import com.sensiblemetrics.api.customiere.actuator.property.WebServiceActuatorSecurityProperty;
import com.sensiblemetrics.api.customiere.commons.exception.SecurityConfigurationException;
...
@Slf4j
@Configuration
@EnableConfigurationProperties(WebServiceActuatorSecurityProperty.class)
@Role(BeanDefinition.ROLE_INFRASTRUCTURE)
@Description("SensibleMetrics Web Service Actuator Security configuration")
public abstract class WebServiceActuatorSecurityConfiguration {
    ...
    @ConditionalOnProperty(prefix = WebServiceActuatorSecurityProperty.PROPERTY_PREFIX, value = "enabled", havingValue = "true")
    @Role(BeanDefinition.ROLE_INFRASTRUCTURE)
    ...
    public static class AuthActuatorSecurityConfigurerAdapter extends WebSecurityConfigurerAdapter {
        private final WebServiceActuatorSecurityProperty property;
     ...
     */
    @Configuration(proxyBeanMethods = false)
    @ConditionalOnProperty(prefix = WebServiceActuatorSecurityProperty.PROPERTY_PREFIX, value = "enabled", havingValue = "false", matchIfMissing = true)
    @Role(BeanDefinition.ROLE_INFRASTRUCTURE)
